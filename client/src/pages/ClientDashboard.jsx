...
export default function ClientDashboard() {
  ...
  useEffect(() => { (async () => { try { const token = localStorage.getItem("token"); const response = await fetch(`${API_URL}/bookings/mine`, { headers: { Authorization: `Bearer ${token}` } }); const data = await response.json(); setBookings(response.ok && Array.isArray(data) ? data : []); } finally { setLoading(false); } })(); }, []);
